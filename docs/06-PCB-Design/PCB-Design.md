title: PCB Design
---

## Overview
This PCB design includes all components from the schematic. The motor will have female header pins to allow a custom length from the board to its final location. Additionally, the microcontroller footprint will also have female headers, as it was already constructed due to breadboarding.

PCB Front Layer:
<img width="1430" height="982" alt="image" src="https://github.com/user-attachments/assets/904cf372-10e3-4e94-a5d3-4b8d1845ff9d" />






PCB Back Layer:
<img width="1396" height="972" alt="image" src="https://github.com/user-attachments/assets/dbd6ce28-024e-4bb8-810c-edcb87d79811" />






[Front & Back.pdf](https://github.com/user-attachments/files/23973481/Front.Back.pdf)

[ZIP File With Footprints](https://github.com/user-attachments/files/23468088/MotorSchematicDesign.zip)

[GERBER_Files.zip](https://github.com/user-attachments/files/23978797/Schematic.design_V2.GERBER.zip)


